...
impl Unify {
    ...
    pub fn env(&self) -> Vec<(Rc<str>, Rc<Term>)> {
        self.reg.iter().filter_map(|(k, v)| {
            match &self.vars[*v] {
                Vif::Value(x) => Some((Rc::clone(k), Rc::clone(x))),
                Vif::Index(x) => {
...
// TODO occurs
    ...
    fn follow(&self, mut x : usize) -> (usize, Option<Rc<Term>>) {
        while let Vif::Index(y) = self.vars[x] { x = y; }
        match self.vars[x] {
            Vif::Value(ref v) => (x, Some(Rc::clone(v))),
            Vif::Free => (x, None),
